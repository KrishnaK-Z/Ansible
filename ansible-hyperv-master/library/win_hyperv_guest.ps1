...
$diskspace = Get-Attr -obj $params -name diskspace -default "20GB"
$diskpath = Get-Attr -obj $params -name diskpath -default $null
$iso_file_path  = Get-Attr -obj $params -name iso_file_path -default $null
...
Function VM-Create {
  ...
  if (!$CheckVM) {
    ...
    if ($diskpath) {
      ...
      if (Test-Path $diskpath) {
        $cmd += " -VHDPath '$diskpath'"
          ...
          $cmd += " -NewVHDPath '$diskpath'-NewVHDSizeBytes $diskspace"
        }
      }
    ...
    Function VM-Set-Iso {
      $CheckVM = Get-VM -name $name -ErrorAction SilentlyContinue
      if ((!$iso_file_path) -and (Test-Path $iso_file_path)) {
        Fail-Json $result "iso path not found"
      }
      elseif ($CheckVM) {
          $cmd="Set-VMDvdDrive -VMName $name -Path $iso_file_path"
          $results = invoke-expression $cmd
          $result.changed = $true
      } else {
